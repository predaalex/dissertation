...
\begin{enumerate}
    \item \textbf{Introduction} -- presents the motivation for choosing this topic, the problem addressed by the dissertation, and the main objectives and personal contributions.

    \item \textbf{Background and Related Work} -- introduces the concepts needed to understand the project, including affective computing, facial emotion recognition, public datasets for facial emotion recognition, and existing components used for local AI assistants.

    \item \textbf{Facial Emotion Recognition Model} -- describes the dataset, preprocessing steps, MobileNetV2 model architecture, training pipeline, experimental strategy, and final model selection.

    \item \textbf{System Design and Architecture} -- presents the implemented assistant architecture, including the FastAPI backend, session and emotion state management, webcam emotion integration, speech-to-text integration, prompt construction, LLM response generation, and the end-to-end interaction flow.

    \item \textbf{Conclusions and Future Work} -- summarizes the dissertation, discusses the main results and limitations, and presents possible directions for future development.
\end{enumerate}
