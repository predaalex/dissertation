...
Facial Emotion Recognition is the task of predicting a person's apparent emotion from an image or video frame of the face. Common emotion classes include anger, disgust, fear, happiness, sadness, surprise, and neutrality. In this project, the detected facial emotion is not treated as a certain description of the user's real emotional state. Instead, it is used as additional context for the assistant.
...
For response generation, large language model families provide versions that can be executed locally. Examples include Llama, Gemma, Mistral, Qwen, and Phi \cite{touvron2023llama2,gemma2024}. Tools such as Ollama simplify local deployment by downloading models, exposing them through a local API, and handling inference on the user's machine.
